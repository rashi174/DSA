...
class Solution:
    ...
    def findPath(self, matrix, n):
        if matrix[0][0] == 0 or matrix[n-1][n-1]==0:
            return []
        ans=''
        self.MazeRat(matrix,0,0,ans,n)
        return sorted(self.paths)
        
    def MazeRat(self,matrix,row,col,currentpath,n):
        ...
        directions='UDLR'
        for i in range(4):
            if self.isSafe(matrix,row+lr[i],col+ls[i],n):
                matrix[row][col]=0
                currentpath+=directions[i]
                self.MazeRat(matrix,row+lr[i],col+ls[i],currentpath,n)
                matrix[row][col]=1  #backtrack
                currentpath=currentpath[:-1]
    
    def isSafe(self,matrix,row,col,n):
